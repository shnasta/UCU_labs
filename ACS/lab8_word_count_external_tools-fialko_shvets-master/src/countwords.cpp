...
CountWords::CountWords(std::unique_ptr<countwords_config> config_ptr) :
    ...
    dictionaries_queue.set_capacity(static_cast<long>(dictionaries_queue_size));
...
//            std::cerr << "Failed to open file " << filename << std::endl;
        ...
        if (!s.empty()) {
            raw_files_queue.push(std::move(std::pair<std::string, std::string>{s, filename.extension()}));
        }
    }
...
void CountWords::count_words() {
    while (true) {
        ...
        if (file_content.first.empty() || file_content.second.empty()) {
            dictionaries_queue.push(words_map{}); // end of queue
            break;
        }
...
//                std::cerr << "Failed to open archive!" << std::endl;
            ...
            while (archive_read_next_header(archive, &entry) == ARCHIVE_OK) {
                ...
                dictionaries_queue.push(std::move(temp_map));
            }
...
void CountWords::merge_dictionaries() {
    while (true) {
        words_map temp_map;
        {
            ...
            dictionaries_queue.pop(temp_map);
            if (temp_map.empty()) {
