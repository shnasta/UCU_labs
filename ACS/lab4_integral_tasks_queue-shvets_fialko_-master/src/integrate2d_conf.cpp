...
auto integrate2d_conf::parse_string(std::string arg){
...
void integrate2d_conf::file_to_options_map(std::istream& cf){
    std::string line;
...
void integrate2d_conf::read_conf(std::istream& cf)
{
...
void integrate2d_conf::validate_conf() const{
    if( x2 <= x1 ) {
...
integrate2d_conf::integrate2d_conf(std::istream& fs){
    read_conf(fs);
    options_map = options_map_t{};
}

template<typename T>
T integrate2d_conf::get_option_from_map(const std::string& option_name) const{
